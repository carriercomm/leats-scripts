...
Create a repository named 'Local Repo 2' with ID [local2]
The baseurl has to be 'http://desktop/repository', gpgcheck has to be enabled.
For GPG key set up http://desktop/repository/RPM-GPG-KEY-CentOS-6.";
our $hint="Modify the .repo file under /etc/yum.repos.d/ to deactivate 'local' repo. 
Create the .repo file under /etc/yum.repos.d/ for 'local2' repo.
...
#	Packages::CreateRepo("local.repo","local","Local Repo","http://desktop",0,"",1);
	...
	printS("Checking Repo 'local' exist and disabled","$L");
	Framework::grade(Packages::CheckRepoExist("local","disabled"));

        printS("Checking Repo 'local2' exist and enabled","$L");
        Framework::grade(Packages::CheckRepoExist("local2","enabled"));	

	printS("Checking Repo 'local2' gpgcheck is enabled","$L");
	Framework::grade(Packages::CheckRepoAttribute("local2","gpgcheck","1"));

        printS("Checking Repo 'local2' name is 'Local Repo 2'","$L");
        Framework::grade(Packages::CheckRepoAttribute("local2","name","Local Repo 2"));	

        printS("Checking Repo 'local2' baseurl is http://desktop/repository","$L");
        Framework::grade(Packages::CheckRepoAttribute("local2","baseurl","http://desktop/repository"));
	
	printS("Checking Repo 'local2' gpgkey is http://desktop/repository/RPM-GPG-KEY-CentOS-6","$L");
        Framework::grade(Packages::CheckRepoAttribute("local2","gpgkey","http://desktop/repository/RPM-GPG-KEY-CentOS-6"));
