...
The required package(s) can be found under http://desktop/repository/Packages.
(http://desktop/repository is already configured as a yum repository.)";
our $hint="Find the package what provides '/usr/bin/dig'. (yum)
The yum repository has been configured up, so you can use yum and rpm also.";
#
#
#
#############
...
sub break() {
	...
	&pre(); #Reseting server machine...
 	...
 	Packages::CreateRepo("local.repo","local","Local Repo","http://desktop/repository",0,"",1);
...
sub grade() {
	...
	printS("Checking package has been updated","$L");
	Framework::grade(Packages::CheckPackageInstalled("bind-utils","9.8.2-0.17.rc1.el6"));
