...
Find the package what provides '/bin/traceroute' and remove it.

";
our $hint="Find the package (yum). Remove the package. (rpm or yum)";
#
#
#
#############
...
sub grade() {
	...
	cryptText2File("<ROOT>$USERDATA<DATE>$now</DATE><TOPIC>$topic</TOPIC><PROBLEM>$problem</PROBLEM><DESCRIPTION>$description</DESCRIPTION>","$result_file");
        ...
        printS("Checking package has been removed","$L");
        Framework::grade(!Packages::CheckPackageInstalled("traceroute"));
