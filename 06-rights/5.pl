...
our $hint="Change owner of file to tom. 
Set the SETUID bit to the file (chmod).";
#
#
#
#############
...
sub break() {
	...
	System::CopyFromDesktop("/ALTS/ExerciseScripts/GetUID","/mnt/files/GetUID","755","root","root","");	
...
sub grade() {
	...
	my $File="/mnt/files/GetUID";

	printS("SETUID is set on $File","$L");
        Framework::grade(checkUserFileSpecialPermission("$File","SETUID"));

	printS("The owner is tom","$L");
	Framework::grade(checkOwner("$File","tom"));
